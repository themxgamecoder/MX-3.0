...
import SaveCreds from './mxgamecoder/makesession.js'
import clearTmp from './mxgamecoder/tempclear.js'
...
async function main() {
  ...
  if (!txt) {
    console.error('meka environment variable not found.')
    return
  }
...
async function connectionUpdate(update) {
  ...
  if (code && (code === DisconnectReason.restartRequired || code === 428)) {
    conn.logger.info(chalk.yellow('\n🌀 Restart Required... Restarting MEKA Ai'))
    process.send('reset')
  }
...
const pluginFolder = global.__dirname(join(__dirname, './meka/index'))
